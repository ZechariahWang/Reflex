...
colorized depth, an iPhone (head camera) panel,
...
- `backend/`  - FastAPI + roslibpy. The only thing that talks to ROS. Caches the latest
  ...
  `/ws/camera/realsense/{color,depth}`, `/ws/camera/iphone/color`. Reconnects to rosbridge forever.
  The iPhone is a ROS camera too (`/head_camera`, started with `head_camera:=iphone`).
...
| `REALSENSE_ROTATION` | `270` | degrees clockwise the RealSense picture is turned on the page and for the object detector (the camera sits portrait on the hand); `0` = as sent |
