...
```
(`min_mm`/`max_mm` only on depth.) The RealSense metas also carry `"rotation": 90`: the wrist camera sits portrait on the hand, and `REALSENSE_ROTATION` (0 / 90 / 180 / 270, default 90) says how far the PAGE turns its colour and depth pictures clockwise to stand them upright - the frames themselves, the ROS topics, the recordings and the policy keep the picture as the camera sends it (`width` / `height` of the meta are of that one). The object detector gets the same turn, because it knows upright bottles, and its boxes are put back into the picture as sent before the depth is read. `available: false` = no frame received in the last 2 s.
