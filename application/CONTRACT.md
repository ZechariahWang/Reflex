...
```
(`min_mm`/`max_mm` only on depth.) The RealSense metas also carry `"rotation": 270`: the wrist camera sits portrait on the hand, and `REALSENSE_ROTATION` (0 / 90 / 180 / 270, default 270: found on the hand, 90 stood the picture on its head) says how far the PAGE turns its colour and depth pictures clockwise to stand them upright - the frames themselves, the ROS topics, the recordings and the policy keep the picture as the camera sends it (`width` / `height` of the meta are of that one). The object detector gets the same turn, because it knows upright bottles, and its boxes are put back into the picture as sent before the depth is read. `available: false` = no frame received in the last 2 s.
...
- A step is over when its seconds have passed AND the hand has arrived (every finger of `/hand/state` within 0.04 of the pose): the next pose is never sent before that. A finger that cannot arrive holds the movement for 3 s at most, then it goes on.
- The running one is `movement` in `/ws/state`: `null` or `{"name", "title", "step", "steps", "waiting", "not_arrived"}` (`waiting`: the step's time is over, the hand is not there yet; `not_arrived`: steps that gave up after 3 s).
...
- `src/components/console/console-stage.tsx` -> `export function ConsoleStage()`: the middle row, left to right 3D hand (01) with the command panel (05) in a short strip under it / RealSense over iPhone (02, 03) / mirror (04). The mirror column is on the very right and is on the page ONLY while the Mirror switch is on: it is `MirrorViewport` (the controller's webcam with the tracked skeleton, the guided calibration, controller / command / state bars), which owns the webcam and `/ws/mirror`, so neither exists while the switch is off. With Mirror off the 3D hand takes the room.
- `src/components/telemetry/command-panel.tsx` -> `export function CommandPanel()`: panel 05, the command block alone (sliders, the Arm, Backdrive and Mirror switches, presets) in a short strip under the 3D hand. The per-finger telemetry columns are gone: the hand viewport's fingertip labels and the command block's measured ticks show the same numbers.
- `src/components/console/top-bar.tsx` -> `export function TopBar()`
- `src/app/page.tsx` composes them: top bar; main row = the 3D hand (~70 % wide, full height, the page's centrepiece) with the command panel under it + a right column with the mirror, RealSense and iPhone panels stacked (~30 %). Panels are numbered 01 hand, 02 mirror, 03 RealSense, 04 iPhone, 05 command. `Panel`'s header is a container: a narrow panel drops its topic tag first, then its status word (the dot stays) unless `keepStatusLabel` is set.
