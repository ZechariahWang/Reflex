...
class LinkagePublisher(Node):
    ...
    publishes the six passive joints of each finger next to them, so
    robot_state_publisher (and any viewer that reads /joint_states) shows the
    mechanism as it really moves.
    """
    ...
    def on_joint_states(self, msg):
        ...
        if not all(joint in angle for joint in self.driven):
            return  # our own message coming back, or somebody else's joints
        out = JointState()
        out.header.stamp = msg.header.stamp  # same instant as the horns: no tearing in TF
        out.name = self.names
        out.position = [a for joint, solver in zip(self.driven, self.solvers) for a in solver(angle[joint])]
