...
| `/hand/orientation` | `geometry_msgs/QuaternionStamped` | `base_link` in a z-up world from the wrist camera's IMU (yaw relative to the start). In `/ws/state` as `orientation: [x, y, z, w] \| null` (null when older than 2 s): the viewer turns the hand by its yaw. The object tracker keeps its tracks in that world (camera mount from `hand_params.yaml` `camera.rpy`), so a track does not swing with the hand between two detections. An object that is not detected any more is forgotten after `MEMORY_S` = 1 s, which is the time the page takes to fade it out (`world-layer.tsx`: from 0.4 s to 1.0 s of `age`). The detector reports only the labels of `DETECT_LABELS` in `objects.py` (`bottle` so far); the page gets every object in the camera's frame as it is turned now. Without an orientation (and for objects seen by the head camera) the tracks live in the camera's frame as before. |
